...
export function Component() {
    ...
    return (
        <div className="flex flex-col h-screen"> {/* Container for the chat application */}
            <header className="bg-primary text-primary-foreground py-4 px-6 flex items-center justify-between"> {/* Header with styling */}
                <h1 className="text-2xl font-bold">AI Chat & code-helper</h1> {/* Title */}
            </header>
            <div className="flex-1 overflow-auto p-6"> {/* Main chat area */}
                <div className="space-y-4"> {/* Container for messages */}
                    {messages.map((m) => (
                            ...
                            {m.role === 'user' ? (
                                <>
                                    ...
                                    <Avatar className="w-10 h-10">
                                        <AvatarImage src="/placeholder-user.jpg" /> {/* User avatar image */}
                                ...
                                <>
                                    <Avatar className="w-10 h-10">
                                        <AvatarImage src="/placeholder-user.jpg" /> {/* AI avatar image */}
            ...
            <div className="bg-background border-t px-6 py-4"> {/* Input area */}
                <form onSubmit={handleSubmit} className="relative">
                    <Textarea value={input} onChange={handleInputChange} onKeyDown={handleKeyDown} placeholder="Type your message..." className="w-full rounded-lg pr-16 resize-none text-lg" rows={1} />
                    <Button type="submit" variant="ghost" size="icon" className="bg-primary text-white absolute top-1/2 right-4 -translate-y-1/2">
                        <SendIcon className="w-5 h-5" /> {/* Send button with icon */}
                        <span className="sr-only">Send</span> {/* Screen reader only text */}
                    </Button>
                </form>
            </div>
        </div>
